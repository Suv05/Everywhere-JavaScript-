...
function App() {
  ...
  const handleGenerate = (e) => {
    e.preventDefault();
    const amount = parseInt(getAmount) || 1;
    const newPara = [];
    for (let i = 0; i < amount; i++) {
      newPara.push(text[randPara(text.length)]);
    }
    setPara(newPara);
  };
    ...
    <>
      <section className="section-center">
        ...
        <article className="lorem-text">
          {para && para.length > 0
            ? para.map((item, index) => <p key={index}>{item}</p>)
            : "Enter the no. of paragraph you want to see"}
        </article>
      </section>
    </>
  );
